    ...
    public class ClassWalletSyncSystem
    {
        ...
        private async Task<bool> UpdateWalletSyncFromInternalSyncMode(string walletFileName, CancellationTokenSource cancellation)
        {
            ...
            if (ClassBlockchainDatabase.BlockchainMemoryManagement.GetLastBlockHeight >= BlockchainSetting.GenesisBlockHeight)
            {
                ...
                if (!cancelled)
                {
                    #region Update block transaction cache.

                    using (var listBlockHeight = await DatabaseSyncCache[walletAddress].GetListOfAllBlockTransactionHash(cancellation))
                    {
                        foreach (long blockHeight in listBlockHeight.GetList.Keys)
                        {
                            if (cancellation.IsCancellationRequested)
                                break;
                            ...
                            foreach (string transactionHash in listBlockHeight.GetList[blockHeight])
                            {
                                if (cancellation.IsCancellationRequested)
                                    break;

    ...
    public class ClassPeerNetworkBroadcastInstanceMemPool
    {
        ...
        internal class ClassPeerNetworkClientBroadcastMemPool : ClassPeerSyncFunction, IDisposable
        {
                ...
                TaskManager.TaskManager.InsertTask(new Action(async () =>
                {
                    while (!successConnect)
                    {
                        try
                        {
                            ...
                            if (_peerSocketClient.Connect(_peerIpTarget, _peerPortTarget, _peerNetworkSettingObject.PeerMaxDelayToConnectToTarget * 1000))
                            {
                                successConnect = true;
                                break;
                            }
            ...
            public async Task<bool> TryAskBroadcastMode()
            {
                ...
                if (!await TrySendPacketToPeer((packetSendObject.GetPacketData() + ClassPeerPacketSetting.PacketPeerSplitSeperator).GetByteArray()))
                {
                    IsAlive = false;
                    return false;
                }
                            ...
                            while (!broadcastResponsePacketStatus)
                            {
                                using (ReadPacketData readPacketData = await _peerSocketClient.TryReadPacketData(_peerNetworkSettingObject.PeerMaxPacketBufferSize, cancellationReceiveBroadcastResponsePacket))
                                {
                                    ...
                                    if (listPacketReceived.Count > 0 && containSeperator)
                                    {
                                        ...
                                        ClassPeerPacketRecvObject peerPacketRecvObject = new ClassPeerPacketRecvObject(listPacketReceived[listPacketReceived.Count - 1].Packet.GetByteArray(), out bool status);
                            ...
                            while (!packetReceived && IsAlive && !taskComplete)
                            {
                                using (ReadPacketData readPacketData = await _peerSocketClient.TryReadPacketData(_peerNetworkSettingObject.PeerMaxPacketBufferSize, cancellationReceiveBlockListPacket))
                                {
                                    ...
                                    if (containSeperator)
                                    {
                                        ...
                                        byte[] base64Packet = listPacketReceived[listPacketReceived.Count - 1].Packet.GetByteArray();
                                    ...
                                    while (!endBroadcast && receiveStatus && IsAlive)
                                    {
                                        ...
                                        using (ReadPacketData readPacketData = await _peerSocketClient.TryReadPacketData(_peerNetworkSettingObject.PeerMaxPacketBufferSize, cancellationReceiveTransactionPacket))
                                        {
                                            ...
                                            for (int i = 0; i < listPacketReceived.Count; i++)
                                            {
                                                ...
                                                {
                                                    bool exceptionBase64 = false;
                                                    byte[] base64Data = listPacketReceived[i].Packet.GetByteArray();
                                                            ...
                                                            peerPacketRecvObject.PacketOrder == ClassPeerEnumPacketResponse.SEND_MEM_POOL_END_TRANSACTION_BY_BLOCK_HEIGHT_BROADCAST_MODE)
                                                            {
                                                                ...
                                                                if (peerPacketRecvObject.PacketOrder == ClassPeerEnumPacketResponse.SEND_MEM_POOL_TRANSACTION_BY_BLOCK_HEIGHT_BROADCAST_MODE)
                                                                {
                                                                    ...
                                                                    if (packetTranslated.Status && packetTranslated.PacketTranslated.ListTransactionObject.Count > 0)
                                                                    {
                                                                        ...
                                                                        }.GetPacketData() + ClassPeerPacketSetting.PacketPeerSplitSeperator).GetByteArray()))
                                                                        {
                                                                            IsAlive = false;
                                                                            endBroadcast = true;
                                                                            break;
                        ...
                        using (DisposableList<ClassReadPacketSplitted> listPacketReceived = new DisposableList<ClassReadPacketSplitted>())
                        {
                            ...
                            while (IsAlive && !taskDone && !voteStatus)
                            {
                                ...
                                using (ReadPacketData readPacketData = await _peerSocketClient.TryReadPacketData(_peerNetworkSettingObject.PeerMaxPacketBufferSize, cancellationReceiveMemPoolTransactionVote))
                                {
                                    ...
                                    {
                                        bool exceptionBase64 = false;
                                        byte[] base64Data = listPacketReceived[listPacketReceived.Count - 1].Packet.GetByteArray();

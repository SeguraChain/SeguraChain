                    ...
                    while (_peerNetworkBroadcastInstanceMemPoolStatus)
                    {
                        ...
                        if (ClassPeerDatabase.DictionaryPeerDataObject.Count > 0)
                        {
                            foreach (string peerIpTarget in ClassPeerDatabase.DictionaryPeerDataObject.Keys.ToArray())
                            {
                                ...
                                if (!peerIpTarget.IsNullOrEmpty(out _))
                                {
                                    if (ClassPeerDatabase.DictionaryPeerDataObject[peerIpTarget].Count > 0)
                                    {
                                        foreach (string peerUniqueIdTarget in ClassPeerDatabase.DictionaryPeerDataObject[peerIpTarget].Keys.ToArray())
                                        {
                                            ...
                                            if (!peerUniqueIdTarget.IsNullOrEmpty(out _))
                                            {
        ...
        internal class ClassPeerNetworkClientBroadcastMemPool : ClassPeerSyncFunction, IDisposable
        {
            ...
            public async Task<bool> TryAskBroadcastMode()
            {
                ...
                using (CancellationTokenSource cancellationReceiveBroadcastResponsePacket = CancellationTokenSource.CreateLinkedTokenSource(_peerCancellationToken.Token))
                {
                    ...
                    try
                    {
                                ...
                                using (DisposableList<ClassReadPacketSplitted> listPacketReceived = new DisposableList<ClassReadPacketSplitted>())
                                {
                                    ...
                                    using (NetworkStream networkStream = new NetworkStream(_peerSocketClient))
                                    {
                                        while (!broadcastResponsePacketStatus)
                                        {
                                            ...
                                            {
                                                string packetReceived = listPacketReceived[listPacketReceived.Count - 1].Packet.GetList.ToArray().GetStringFromByteArrayUtf8();

                                                ClassPeerPacketRecvObject peerPacketRecvObject = new ClassPeerPacketRecvObject(Convert.FromBase64String(packetReceived), out bool status);
                        ...
                        }, cancellationReceiveBroadcastResponsePacket.Token, TaskCreationOptions.RunContinuationsAsynchronously, TaskScheduler.Current);
                                        ...
                                        foreach (long blockHeight in listMemPoolBlockHeight.GetList)
                                        {
                                            ...
                                            if (countMemPoolTransaction > 0)
                                            {
                                                ...
                                                using (DisposableList<ClassTransactionObject> listTransactionObjectToSend = await ClassMemPoolDatabase.GetMemPoolTxObjectFromBlockHeight(blockHeight, true, _peerCancellationToken))
                                                {
                                                    ...
                                                    var listTransactionResult = await TryReceiveMemPoolTransactionVotePacket();
            ...
            private void EnableCheckConnect()
            {
                try
                {
                    ...
                    }, _peerCheckConnectionCancellationToken.Token, TaskCreationOptions.RunContinuationsAsynchronously, TaskScheduler.Current);
                }
                catch
                {
                    // Ignored, catch the exception once the task is cancelled.
                ...
                using (CancellationTokenSource cancellationReceiveBlockListPacket = CancellationTokenSource.CreateLinkedTokenSource(_peerCancellationToken.Token))
                {
                    try
                    {
                        taskReceivedBlockHeight = Task.Factory.StartNew(async () =>
                        {
                            ...
                            taskComplete = true;
                        }, cancellationReceiveBlockListPacket.Token, TaskCreationOptions.RunContinuationsAsynchronously, TaskScheduler.Current);
                                                    ...
                                                    if (containSeperator)
                                                    {
                                                        for (int i = 0; i < listPacketReceived.Count; i++)
                                                        {
                                                            ...
                                                            if (listPacketReceived[i].Complete && listPacketReceived[i].Packet.Count > 0)
                                                            {
                                                                ...
                                                                try
                                                                {
                                                                    base64Data = Convert.FromBase64String(listPacketReceived[i].Packet.GetList.ToArray().GetStringFromByteArrayUtf8());
                                                                }
                                                                catch
                                                                {
                                                                    exceptionBase64 = true;
            ...
            /// </summary>
            /// <returns></returns>
            private async Task<Dictionary<string, ClassTransactionEnumStatus>> TryReceiveMemPoolTransactionVotePacket()
            {
                bool voteStatus = false;
                bool taskDone = false;
                long timestampStart = ClassUtility.GetCurrentTimestampInMillisecond();
                Dictionary<string, ClassTransactionEnumStatus> listTransactionStatus = new Dictionary<string, ClassTransactionEnumStatus>();
                                    ...
                                    using (NetworkStream networkStream = new NetworkStream(_peerSocketClient))
                                    {
                                        while (IsAlive && !taskDone && !voteStatus)
                                        {
                                            ...
                                            if (containSeperator)
                                            {
                                                ...
                                                try
                                                {
                                                    base64Data = Convert.FromBase64String(listPacketReceived[listPacketReceived.Count - 1].Packet.GetList.ToArray().GetStringFromByteArrayUtf8());
                                                }
                                                catch
                                                {
                                                    exceptionBase64 = true;
                                                        ...
                                                        if (status)
                                                        {
                                                            if (peerPacketRecvObject.PacketOrder == ClassPeerEnumPacketResponse.SEND_MEM_POOL_TRANSACTION_VOTE)
                                                            {
                                                                ...
                                                                if (packetTranslated.Status)
                                                                {
                                                                    ...
                                                                    listTransactionStatus = packetTranslated.PacketTranslated.ListTransactionHashResult;

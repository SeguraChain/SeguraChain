            ...
            try
            {
                if (GetTotalActiveConnectionFromClientIp(clientIp) < _peerNetworkSettingObject.PeerMaxApiConnectionPerIp)
                {
                    ...
                    if (_listApiIncomingConnectionObject.Count < int.MaxValue - 1)
                    {
                        ...
                        {
                            if (!_listApiIncomingConnectionObject[clientIp].ListApiClientObject.TryAdd(randomId, new ClassPeerApiClientObject(_peerDatabase, clientApiTcp, clientIp, _peerNetworkSettingObject, _firewallSettingObject, _peerIpOpenNatServer, _cancellationTokenSourcePeerApiServer)))
